...
// Sort abbrs.json
{
	...
	writeFileSync(
		path,
		JSON.stringify(
			abbrs.sort((a, b) => (a.word < b.word ? -1 : 1)),
			null,
			3,
		),
		'utf-8',
...
// Mirror abbrs.json words in translations.json for each language
{
	for (const lang of langs) {
		...
		for (const abbr of abbrs) {
			const abbrTranslation = currTranslations.find(
				(translation) => abbr.word === translation.word,
			);
...
// Sort translations
{
	for (const lang of langs) {
		...
		writeFileSync(
			path,
			JSON.stringify(
				translations.sort((a, b) => (a.word < b.word ? -1 : 1)),
				null,
				3,
			),
			'utf-8',
